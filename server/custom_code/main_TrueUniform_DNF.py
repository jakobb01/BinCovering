"""
Driver for running DNF strategy with True Uniform Generator.
Generates items with true uniform distribution (no OPT known),
applies swaps, runs DNF, and logs results to CSV.
Plots: 1) DNF bins filled vs swaps, 2) Item distribution, 3) Result size class histogram.
"""
import sys
import datetime
import os
...
def plot_results(csv_path, items_list, output_path=None):
    ...
    Shows: 1) Item size distribution, 2) DNF bins filled vs swaps, 3) Result size class histogram.
    ...
    # Plot 2: DNF Bins Filled vs Swaps - MIDDLE
    ax2.plot(swaps, bins_filled_list, 'b-', linewidth=0.5, alpha=0.7, label='DNF Bins Filled')
    ax2.scatter(swaps, bins_filled_list, s=1, c='blue', alpha=0.5)
    
    ax2.set_xlabel('Number of Swaps', fontsize=12)
    ax2.set_ylabel('Bins Filled (DNF)', fontsize=12)
    ax2.set_title('DNF Strategy Performance with True Uniform Distribution\n(Bins Filled vs Number of Swaps)', fontsize=14)
    ...
    # Plot the histogram
    if class_range == 0:
        ax3.bar(class_labels, list(class_counts.values()), color='coral', edgecolor='black', alpha=0.7)
    else:
...
def run_true_uniform_dnf_experiment(num_items=1000, swap_step=1, max_swaps=None, data_path="./data/"):
    """
    Run the DNF experiment with true uniform distribution generator.
    
    Args:
        num_items: number of items to generate
        swap_step: step size for number of swaps (default 1)
        max_swaps: maximum number of swaps (default = num_items)
    ...
    """
    print(f"\n{'='*60}")
    print(f"Running True Uniform DNF Experiment")
    print(f"Items: {num_items}, Swap step: {swap_step}")
    print(f"{'='*60}\n")
    ...
    # Set max_swaps to 16 * number of items if not specified
    if max_swaps is None:
        max_swaps = num_items * 16
    
    # Create CSV filename
    timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    csv_filename = f"true_uniform_dnf_results_{num_items}items_{timestamp}.csv"
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        fieldnames = ['experiment_id', 'num_items', 'swaps', 'bins_filled', 'timestamp']
        ...
        for swaps in range(0, max_swaps + 1, swap_step):
            ...
            if experiment_id % max(1, total_experiments // 10) == 0:
                ...
                print(f"Progress: {progress:.1f}% ({experiment_id}/{total_experiments}) - Swaps: {swaps}, Bins: {bins_filled}")
    ...
    if PLOTTING_AVAILABLE:
        plot_filename = f"true_uniform_dnf_plot_{num_items}items_{timestamp}.png"
...
def main():
    """Main entry point for the true uniform DNF driver."""
    import argparse
    
    parser = argparse.ArgumentParser(
        description='Run DNF strategy with true uniform distribution generator (no OPT)'
    )
    parser.add_argument(
        '-n', '--num-items',
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum number of swaps (default: 16 * num-items)'
    )
    parser.add_argument(
        '-p', '--path',
        type=str,
