"""
Driver for running ThrowBin_DNF strategy with Uniform Generator.

ThrowBin_DNF maintains exactly 10 active bins at all times by replacing covered bins with new empty ones.

Usage:
    python3 custom_code/main_ThrowBin_DNF.py -n 1000 -r 1000
    python3 custom_code/main_ThrowBin_DNF.py --num-items 10000 --runs 10000
"""
import sys
import datetime
import os
...
from bincovering.algorithms.ThrowBin_DNF import ThrowBinDNFStrategy
...
def run_single_experiment(items):
    """Run a single ThrowBin_DNF experiment."""
    gen = InMemoryGenerator(items)
    gen.start()
    
    strat = ThrowBinDNFStrategy()
...
def plot_results(csv_path, items_list, output_path=None, num_items=None):
    ...
    # Add reference line for active bins (always 10)
    ...
    ax2.set_title('ThrowBin_DNF Strategy Performance\n(Total Covered Bins per Run, with 10 constant active bins)', fontsize=14)
    ...
    ax3.set_title('Distribution of ThrowBin_DNF Results', fontsize=14)
...
def run_throwbin_dnf_experiment(num_items=1000, num_runs=None, data_path="./data/"):
    """Run the ThrowBin_DNF experiment."""
    if num_runs is None:
        num_runs = num_items
    
    print(f"\n{'='*60}")
    print(f"Running ThrowBin_DNF Experiment")
    print(f"Items: {num_items}, Active Bins: 10 (constant), Runs: {num_runs}")
    print(f"{'='*60}\n")
    ...
    csv_filename = f"throwbin_dnf_results_{num_items}items_{timestamp}.csv"
    csv_path = os.path.join(data_path, csv_filename)
    
    print(f"\nRunning {num_runs} experiments...")
    
    with open(csv_path, 'w', newline='') as csvfile:
        fieldnames = ['run_id', 'num_items', 'num_active_bins', 'bins_covered', 'coverage_ratio', 'timestamp']
        ...
        for run_id in range(num_runs):
            bins_covered = run_single_experiment(items_list)
            coverage_ratio = bins_covered / num_active_bins
    ...
    print(f"Total items: {num_items}, Active bins: 10, Total runs: {num_runs}")
    
    if PLOTTING_AVAILABLE:
        plot_filename = f"throwbin_dnf_plot_{num_items}items_{timestamp}.png"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Run ThrowBin_DNF strategy (10 constant active bins with replacement)'
    )
    parser.add_argument('-n', '--num-items', type=int, default=1000, 
                       help='Number of items (N) (default: 1000)')
    ...
    if args.plot_only:
        ...
        csv_path = run_throwbin_dnf_experiment(
