...
class ThrowBin_1_Strategy(Strategy):
    ...
    def start(self, generator, num_items, bin_ratio=None):
        ...
        if self.file:
            self.file.write(f"Starting ThrowBin_1 Strategy with {self.num_bins} bins (constant)\n")
    ...
    def stop(self):
        ...
        if self.file:
            ...
            if self.num_bins > 0:
                self.file.write(f"Covered/Active ratio: {self.covered_bins / self.num_bins:.2f}\n")
            self.file.close()
