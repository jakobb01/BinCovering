...
ALIASES = {
    "dnf": "dual_next_fit", "DNF_1": "dual_next_fit",
    "harmonic": "dual_harmonic", "ThrowBin": "throwbin_retire",
    "ThrowBin_1": "throwbin_replace", "ThrowBin_DNF": "throwbin_fixed_active",
    "AdaptiveBin": "adaptive_items", "AdaptiveBinCovered": "adaptive_covered",
}
PARAMETERS = {
    "dual_next_fit": {}, "dual_harmonic": {"k": 5},
    "throwbin_retire": {"bin_ratio": 0.5},
    "throwbin_replace": {"bin_ratio": 0.5},
    "throwbin_fixed_active": {}, "adaptive_items": {},
...
def normalize(spec):
    ...
    if params.keys() - PARAMETERS[name].keys():
        raise ValueError(f"Unknown parameters for {name}: {params.keys() - PARAMETERS[name].keys()}")
    ...
    if 'initial_bins' in params and (type(params['initial_bins']) is not int or params['initial_bins'] < 1):
        raise ValueError("initial_bins must be a positive integer")
    if 'multiplier' in params and (not math.isfinite(params['multiplier']) or params['multiplier'] <= 0):
        raise ValueError("multiplier must be finite and positive")
    backend = spec.get('backend', 'python')
    if backend not in ('python', 'cpp') or (backend == 'cpp' and name not in ('dual_next_fit','dual_harmonic')):
...
def solve(items, spec, threshold, seed, trace=None, cancelled=lambda: False):
    ...
    constructors = {
        'throwbin_retire': ThrowBinStrategy, 'throwbin_replace': ThrowBin_1_Strategy,
        'throwbin_fixed_active': ThrowBinDNFStrategy,
        'adaptive_items': AdaptiveBinStrategy, 'adaptive_covered': AdaptiveBinCoveredStrategy,
    }
