...
class AdaptiveBinCoveredStrategy(Strategy):
    ...
    def _update_bins_based_on_formula(self):
        ...
        if needed > 0:
            if self.file:
                self.file.write(
                    f"Formula update: Covered={self.covered_bins}, Mult={self.multiplier}, Target={target_total}, Current={current_total}, Opening={needed}\\n"
                )
    ...
    def start(self, generator):
        ...
        if self.file:
            self.file.write("Starting AdaptiveBinCovered Strategy\\n")
            self.file.write(
                f"Initial bins: {self.initial_bins}, Multiplier: {self.multiplier}\\n"
            )
    ...
    def next(self):
        ...
        if self.file:
            ...
            self.file.write(
                f"Active: {len(self.active_bin_indices)}, Covered: {self.covered_bins}\\n"
            )
        ...
        if self.bins[selected_idx] >= BIN_COVER_LOAD:
            ...
            if self.file:
                self.file.write(
                    f"Bin {selected_idx} COVERED! Total covered: {self.covered_bins}\\n"
                )
