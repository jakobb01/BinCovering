...
@pytest.mark.parametrize('items, expected', [([],0),([1],0),([400000,600000],1),([700000,700000,600000],1)])
...
def test_harmonic_third_boundary():
    ...
    assert solve([333333,333334,333333],normalize({'id':'harmonic'}),1000000,1).covered_bins == 0


def test_retire_and_replace_are_distinct():
    items=[0.6]*10
    a=solve(items,normalize({'id':'ThrowBin','params':{'bin_ratio':0.1}}),1.0,1)
    b=solve(items,normalize({'id':'ThrowBin_1','params':{'bin_ratio':0.1}}),1.0,1)
