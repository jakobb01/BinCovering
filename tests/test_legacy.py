...
def test_mixer_percentage(tmp_path,monkeypatch):
    spec=importlib.util.spec_from_file_location('legacy_mixer','python_graphs/mixer.py')
    module=importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
    counts=[]
    monkeypatch.setattr(module,'randomize_sort',lambda numbers,amount: counts.append(amount) or numbers)
    module.reorder_numbers([3,2,1],tmp_path/'mixed.txt',5,100)
    assert counts==[3]


@pytest.mark.parametrize('name,module_name,class_name,params',[
...
def test_adapter_preserves_migrated_strategy(name,module_name,class_name,params):
    ...
    cls=getattr(importlib.import_module('bincovering.algorithms.'+module_name),class_name)
    ...
    while original.next() is not None: pass
    assert solve(items,normalize({'id':name,'params':params}),1.0,41).covered_bins==original.covered_bins
